...
- **Breaking:** `BaseRepository::update()` (and `WriteRepositoryContract::update()`) gained a `bool $quiet = false` parameter. `quiet: true` delegates to Eloquent's own `Model::updateQuietly()` (the same `withoutEvents()` mechanism Laravel itself uses for `saveQuietly()`/`deleteQuietly()`) and correspondingly skips the cache flush — an explicit, opt-in "silent update" for columns nothing cached or observed depends on.
